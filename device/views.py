...
import json
from my_helpers import dd, any_to_json, getVlansFromOutput, guestPing, guestShowIp, prepareTelnet, sendGuestIpAddress
import requests
from netmiko import ConnectHandler
from telnetlib import Telnet
from .forms import AddStaticRouteForm, DeviceForm, CreateVlanForm, GuestIpAddressForm
import time

@login_required(login_url='/login')
def deviceStart(request, connection_id, project_id, device_id):
...
@login_required(login_url='/login')
def deviceReload(request, connection_id, project_id, device_id):
    connection = Connection.objects.get(id=connection_id)
    url = f"http://{connection.ip_address}:{connection.port}"
    try:
        requests.adapters.DEFAULT_RETRIES = 5
...
@login_required(login_url='/login')
def deviceSuspend(request, connection_id, project_id, device_id):
    connection = Connection.objects.get(id=connection_id)
    url = f"http://{connection.ip_address}:{connection.port}"
    try:
        requests.adapters.DEFAULT_RETRIES = 5
...
@login_required(login_url='/login')
def deviceStop(request, connection_id, project_id, device_id):
    connection = Connection.objects.get(id=connection_id)
    url = f"http://{connection.ip_address}:{connection.port}"
    try:
        requests.adapters.DEFAULT_RETRIES = 5
...
@login_required(login_url='/login')
def deviceDelete(request, connection_id, project_id, device_id):
    connection = Connection.objects.get(id=connection_id)
    url = f"http://{connection.ip_address}:{connection.port}"
    try:
        requests.adapters.DEFAULT_RETRIES = 5
...
@login_required(login_url='/login')
def deviceIndex(request, connection_id, project_id, device_id):
...
@login_required(login_url='/login')
def deviceRunningConfig(request, connection_id, project_id, device_id):
    connection = Connection.objects.get(id=connection_id)
    url = f"http://{connection.ip_address}:{connection.port}"
    try:
        requests.adapters.DEFAULT_RETRIES = 5
...
@login_required(login_url='/login')
def deviceIpRoute(request, connection_id, project_id, device_id):
    connection = Connection.objects.get(id=connection_id)
    url = f"http://{connection.ip_address}:{connection.port}"
    try:
        requests.adapters.DEFAULT_RETRIES = 5
...
@login_required(login_url='/login')
def deviceStartupConfig(request, connection_id, project_id, device_id):
    connection = Connection.objects.get(id=connection_id)
    url = f"http://{connection.ip_address}:{connection.port}"
    try:
        requests.adapters.DEFAULT_RETRIES = 5
...
@login_required(login_url='/login')
def deviceGetVlans(request, connection_id, project_id, device_id):
    connection = Connection.objects.get(id=connection_id)
    url = f"http://{connection.ip_address}:{connection.port}"
    try:
        requests.adapters.DEFAULT_RETRIES = 5
...
@login_required(login_url='/login')
def devicePingIpAddress(request, connection_id, project_id, device_id, ip_address):
    connection = Connection.objects.get(id=connection_id)
    url = f"http://{connection.ip_address}:{connection.port}"
    try:
        requests.adapters.DEFAULT_RETRIES = 5
...
@login_required(login_url='/login')
def deviceCopyRunningToStartup(request, connection_id, project_id, device_id):
    connection = Connection.objects.get(id=connection_id)
    url = f"http://{connection.ip_address}:{connection.port}"
    try:
        requests.adapters.DEFAULT_RETRIES = 5
...
@login_required(login_url='/login')
def deviceCreateVlan(request, connection_id, project_id, device_id):
    connection = Connection.objects.get(id=connection_id)
    url = f"http://{connection.ip_address}:{connection.port}"
    try:
        requests.adapters.DEFAULT_RETRIES = 5
...
@login_required(login_url='/login')
def deviceVlanAccessMode(request, connection_id, project_id, device_id):
    connection = Connection.objects.get(id=connection_id)
    url = f"http://{connection.ip_address}:{connection.port}"
    try:
        requests.adapters.DEFAULT_RETRIES = 5
...
@login_required(login_url='/login')
def deviceAddStaticRoute(request, connection_id, project_id, device_id):
    connection = Connection.objects.get(id=connection_id)
    url = f"http://{connection.ip_address}:{connection.port}"
    try:
        requests.adapters.DEFAULT_RETRIES = 5
...
@login_required(login_url='/login')
def deviceInterfacesIpAddress(request, connection_id, project_id, device_id):
    connection = Connection.objects.get(id=connection_id)
    url = f"http://{connection.ip_address}:{connection.port}"
    try:
        requests.adapters.DEFAULT_RETRIES = 5
...
@login_required(login_url='/login')
def deviceGuestIpAddress(request, connection_id, project_id, device_id):
    connection = Connection.objects.get(id=connection_id)
    url = f"http://{connection.ip_address}:{connection.port}"
    try:
        requests.adapters.DEFAULT_RETRIES = 5
...
@login_required(login_url='/login')
def deviceGuestShowIp(request, connection_id, project_id, device_id):
    connection = Connection.objects.get(id=connection_id)
    url = f"http://{connection.ip_address}:{connection.port}"
    try:
        requests.adapters.DEFAULT_RETRIES = 5
